...
class CoconBlock(nn.Module):
    ...
    def forward(self, x, context_seq=None, history_seq=None, layer_past=None, attention_mask=None, head_mask=None,
        ...
        output_attn = self.cocon_attn(
            x_1_output, context_seq, layer_past=layer_past, attention_mask=attention_mask, head_mask=head_mask,
...
class CoconAttention(nn.Module):
    ...
    def _attn(self, q, k, v, attention_mask=None, head_mask=None, cs_self_attn_mask_prob=0, history_seq_len=None,
              context_seq_present=True, context_seq_len=0, context_attn_bias=0, context_seq_len_list=None):
        w = torch.matmul(q, k)
        if self.scale:
            w = w / math.sqrt(v.size(-1))
        nd, ns = w.size(-2), w.size(-1)
    ...
    def forward(self, x, context_seq, layer_past=None, attention_mask=None, head_mask=None, cs_self_attn_mask_prob=0,
                history_seq_len=None, context_attn_bias=0, context_seq_len_list=None):
        x = self.c_attn(x)
        query, key, value = x.split(self.split_size, dim=2)
        ...
        present = torch.stack((key.transpose(-2, -1), value))  # transpose to have same shapes for stacking
        attn_outputs = self._attn(query, prepended_key, prepended_value, attention_mask, head_mask,
                                  cs_self_attn_mask_prob=cs_self_attn_mask_prob, history_seq_len=history_seq_len,
                                  context_seq_present=context_seq_present,
                                  context_seq_len=context_seq_len, context_attn_bias=context_attn_bias,
                                  context_seq_len_list=context_seq_len_list)
...
class GPT2Model(GPT2PreTrainedModel):
    def __init__(self, config, output_meanvars=False, compute_meanvars_before_layernorm=False):
        ...
        self.h = nn.ModuleList([Block(config.n_ctx, config, scale=True, output_meanvars=output_meanvars,
            ...
            if input_before_block_ind is not None:
                if input_point == 'current_block_ln_1' and block_ind == input_before_block_ind:
                    ...
                    outputs = block(
                        ...
                        head_mask=head_mask[block_ind], input_point='ln_1'
                    )
                ...
                if output_after_block_ind is not None and (
                    ...
                    hidden_states = block(
                        hidden_states, layer_past=layer_past, attention_mask=attention_mask,
                        head_mask=head_mask[block_ind], return_point='ln_1'
                    )
...
class GPT2LMHeadModel(GPT2PreTrainedModel):
    def __init__(self, config, output_meanvars=False, compute_meanvars_before_layernorm=False):
        super().__init__(config)
        self.transformer = GPT2Model(config, output_meanvars=output_meanvars,
